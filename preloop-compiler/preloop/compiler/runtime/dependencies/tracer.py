...
class Tracer:
    ...
    def __init__(
        ...
        self.watch = [v if isinstance(v, BaseVariable) else CommonVariable(v) for v in utils.ensure_tuple(watch)] + [
            v if isinstance(v, BaseVariable) else Exploding(v) for v in utils.ensure_tuple(watch_explode)
        ]
        ...
        self.objects_to_track.extend(tracking_list)
        self.mode = mode
        self.output_directory = output_dir
        self.file_name = file_name
        self.script_path = script_path
        self.last_line_number = None
        self.loop_line_numbers = os.getenv("LOOP_LINE_NUMBERS")
        if self.loop_line_numbers:
            self.loop_line_numbers = set(map(int, self.loop_line_numbers.split(",")))

        with open(script_path, "r") as file:
            source_code = file.read()

        self.parsed_ast = ast.parse(source_code)
    ...
    def __exit__(self, exc_type, exc_value, exc_traceback):
        ...
        with open(self.output_directory + self.file_name, "wb") as f:
    ...
    def trace(self, frame, event, arg):
        ...
        if not (frame.f_code in self.target_codes or frame in self.target_frames):
            if self.depth == 1:
                ...
                return None
            if self.loop_line_numbers and frame.f_lineno in self.loop_line_numbers:
                return None
        ...
        if self.thread_info:
            ...
            thread_info = "{ident}-{name} ".format(ident=current_thread.ident, name=current_thread.name)
        ...
        if event == "line":
            if not self.derivative_variables:
                for key, value in self.derivative_variables.items():
                    if key in frame.f_locals.items():
                        self.variable_dict[key]["object_type"] = type(frame.f_locals[key])
                        del self.derivative_variables[key]

                    else:
                        raise ValueError("Incorrect script parsing.")

            for key, value in frame.f_locals.items():
                if (source_path == self.script_path) and (isinstance(value, tuple(self.objects_to_track))):
                    if key not in self.variable_dict:
                        is_model = utils.identify_model(value)
                        is_data = utils.check_data_type(value)
                        is_cv = utils.check_cv(value)

                        if is_model:
                            is_model["variable_category"] = "model"
                            self.variable_dict[key] = is_model

                        elif is_cv:
                            items_to_insert = {}
                            items_to_insert["object_type"] = "Grid Search CV"
                            items_to_insert["variable_category"] = "model_selection"
                            items_to_insert["initialized_on"] = self.last_line_number
                            self.variable_dict[key] = items_to_insert

                        elif is_data:
                            ast_node = utils.find_ast_node_at_lineno(self.parsed_ast, self.last_line_number)
                            details = utils.analyze_node(ast_node)
                            item_to_insert = {
                                "package_name": is_data["package_name"],
                                "object_type": is_data["object_type"],
                            ...
                            self.variable_dict[key] = item_to_insert

                    elif key in self.variable_dict:
                        if (self.variable_dict[key]["variable_category"] == "data") or (
                            self.variable_dict[key]["variable_category"] == "derived_data"
                        ):
                            ast_node = utils.find_ast_node_at_lineno(self.parsed_ast, self.last_line_number)
                            details = utils.analyze_node(ast_node)
                            if "variable_changed_or_assigned" in details:
                                if details["variable_changed_or_assigned"] == key:
                                    self.variable_dict[key]["operation_sequence"].append(details)

                                elif key in details["other_variables"]:
                                    item_to_insert = {
                                        "object_type": None,
                                        "variable_category": "derived_data",
                                        "initialized_on": {"line_number": line_no, "initialization_details": details},
                                        "operation_sequence": [],
                                    }
                                    self.variable_dict[details["variable_changed_or_assigned"]] = item_to_insert
                                    self.derivative_variables[details["variable_changed_or_assigned"]] = True
        ...
        ended_by_exception = event == "return" and arg is None and opcode.opname[code_byte] not in RETURN_OPCODES

        if self.mode != "silent":
            if ended_by_exception:
                self.write("{_FOREGROUND_RED}{indent}Call ended by exception{_STYLE_RESET_ALL}".format(**locals()))
            else:
                self.write(
        ...
        if event == "return":
            if source_path == self.script_path:
                for key, value in frame.f_locals.items():
                    is_cv = utils.check_cv(value)
                    is_model = utils.identify_model(value)
                    if is_model != None:
                        if is_model["package_name"] == "scikit-learn":
                            self.variable_dict[key]["params"] = value.get_params()

                        elif is_model["package_name"] == "xgboost":
                            self.variable_dict[key]["params"] = value.get_params()
                    ...
                    if is_cv != None:
                        if "state" in is_cv:
                            ...
                            self.variable_dict[key]["details"] = is_cv
